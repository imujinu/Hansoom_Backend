...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
        return httpSecurity
                ...
                .authorizeHttpRequests(a -> a.requestMatchers("/user/create", "/user/login", "/user/auth/refresh", "/user/google/login", "/user/google/reLogin", "/user/kakao/login", "/payment/**","/reservation/**", "/hotel/detail/**", "/hotel/list", "/hotel/nearby", "/connect/**", "/review/hotel/**", "/review/images/**", "/reply/hotels/**", "/review/ratings/**", "/hotel/popular", "hotel/place").permitAll().anyRequest().authenticated())
                .build();
    }
    // Todo - 프론트 연결
